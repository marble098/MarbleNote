...
class ReminderRepository @Inject constructor(
    ...
    val reminders: Flow<List<Reminder>> = reminderDao.observeAll().map { list -> list.map(ReminderEntity::toModel) }
    val categories: Flow<List<Category>> = categoryDao.observeAll().map { list -> list.map(CategoryEntity::toModel) }
    ...
    suspend fun saveReminder(value: Reminder) {
        ...
        if (normalized.status == ReminderStatus.ACTIVE) scheduler.schedule(normalized) else scheduler.cancel(normalized.id)
    ...
    suspend fun delete(id: String) {
        val old = reminderDao.byId(id)?.toModel() ?: return
        val now = System.currentTimeMillis()
        reminderDao.upsert(ReminderEntity.fromModel(old.copy(updatedAtMillis = now, deletedAtMillis = now)))
        scheduler.cancel(id)
        work.enqueueCloudSync(); work.updateWidgets()
    }

    suspend fun snooze(id: String, minutes: Int = 10) {
        val old = reminderDao.byId(id)?.toModel() ?: return
        saveReminder(old.copy(dueAtMillis = System.currentTimeMillis() + minutes * 60_000L, status = ReminderStatus.ACTIVE))
    }

    suspend fun saveCategory(value: Category) {
        val now = System.currentTimeMillis()
        val normalized = value.copy(id = value.id.ifBlank { UUID.randomUUID().toString() }, createdAtMillis = value.createdAtMillis.takeIf { it > 0 } ?: now, updatedAtMillis = now, deletedAtMillis = null)
        categoryDao.upsert(CategoryEntity.fromModel(normalized)); work.enqueueCloudSync()
    }

    suspend fun get(id: String): Reminder? = reminderDao.byId(id)?.toModel()
    suspend fun upcoming(limit: Int = 8): List<Reminder> = reminderDao.upcoming(System.currentTimeMillis()).take(limit).map(ReminderEntity::toModel)

    suspend fun snapshot(deviceId: String): CloudSnapshot = CloudSnapshot(
        generatedAtMillis = System.currentTimeMillis(), deviceId = deviceId,
    ...
    suspend fun merge(snapshot: CloudSnapshot) = db.withTransaction {
        ...
        val mergedReminders = snapshot.reminders.mapNotNull { remote ->
            ...
            if (local == null || remote.updatedAtMillis > local.updatedAtMillis) ReminderEntity.fromModel(remote) else null
        }
        ...
        val mergedCategories = snapshot.categories.mapNotNull { remote ->
            ...
            if (local == null || remote.updatedAtMillis > local.updatedAtMillis) CategoryEntity.fromModel(remote) else null
        }
        if (mergedCategories.isNotEmpty()) categoryDao.upsertAll(mergedCategories)
    }

    suspend fun rescheduleAll() { upcoming(200).forEach { scheduler.schedule(it) } }
}
